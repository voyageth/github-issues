...
function zipAssets () {
  ...
  const zipAssets = [{
    name: 'github-issues-mac.zip',
    path: path.join(outPath, 'Github Issues-darwin-x64', 'Github Issues.app')
  }, {
    name: 'github-issues-windows.zip',
    path: path.join(outPath, 'github-issues-win32-ia32')
  }, {
    name: 'github-issues-linux.zip',
    path: path.join(outPath, 'Github Issues-linux-x64')
  }]
  ...
  return Promise.all(zipAssets.map(zipAsset)).then((zipAssets) => {
    return zipAssets.concat([{
      ...
      name: 'github-issuesSetup.exe',
      path: path.join(outPath, 'windows-installer', 'github-issuesSetup.exe')
    }, {
      name: `github-issues-${version}-full.nupkg`,
      path: path.join(outPath, 'windows-installer', `github-issues-${version}-full.nupkg`)
    }])
  })
}
...
function createRelease (assets) {
  const options = {
    uri: 'https://api.github.com/repos/voyageth/github-issues/releases',
    headers: {
